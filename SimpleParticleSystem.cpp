...
const char *WINDOWTITLE = { "Particle System -- Kevin Neiger" };
...
struct particle
{
	...
	float tr, tg, tb;     // Target color
	float t00, t0, t1;    // birth time, death time
	float x, y, z;        // current location
...
#define INITREDLOW 0.0
#define INITREDHIGH 0.3
#define INITBLUELOW 0.75
#define INITBLUEHIGH 1.0
#define INITGREENLOW 0.0
#define INITGREENHIGH 0.3
...
Animate( void )
{
	...
	for (i = 0; i < NUMPARTICLES; i++) {
		if (Particles[i].t0 <= Time && Time <= Particles[i].t1) {
			...
			dt = Time - Particles[i].t0;
			Particles[i].x = Particles[i].x0 + Particles[i].vx * dt;
			Particles[i].y = Particles[i].y0 + Particles[i].vy * dt + 0.5 * GRAVITY * dt * dt;
			Particles[i].z = Particles[i].z0 + Particles[i].vz * dt;
			duration = Particles[i].t1 - Particles[i].t0;
			lifeCycle = dt / duration;
			Particles[i].r = Particles[i].r0 + (Particles[i].tr - Particles[i].r0) * lifeCycle;
			Particles[i].g = Particles[i].g0 + (Particles[i].tg - Particles[i].g0) * lifeCycle;
			Particles[i].b = Particles[i].b0 + (Particles[i].tb - Particles[i].b0) * lifeCycle;
			//Particles[i].vy = Particles[i].vy + GRAVITY * Dt;
			
		} else {
			Particles[i].x = Particles[i].x0;
			Particles[i].y = Particles[i].y0;
			Particles[i].z = Particles[i].z0;
			Particles[i].vx = Particles[i].vx0;
			Particles[i].vy = Particles[i].vy0;
			Particles[i].vz = Particles[i].vz0;
			Particles[i].r = Particles[i].r0;
			Particles[i].g = Particles[i].g0;
			Particles[i].b = Particles[i].b0;
			Particles[i].t0 = Time;
			Particles[i].t1 = Ranf(0.f, DURATION) + Time;
		}
	}
...
InitGraphics( void )
{
	...
	// ****************************************
	Particles = new struct particle[NUMPARTICLES];
	for (int i = 0; i < NUMPARTICLES; i++) {
		Particles[i].x0 = Ranf(-10.f, 10.f);
		Particles[i].y0 = Ranf(-10.f, 10.f);
		Particles[i].z0 = Ranf(-10.f, 10.f);
		Particles[i].vx0 = Ranf(-INITIALSPEED, INITIALSPEED);
		Particles[i].vy0 = Ranf(-INITIALSPEED, INITIALSPEED);
		Particles[i].vz0 = Ranf(-INITIALSPEED, INITIALSPEED);
		Particles[i].r0 = Ranf(INITREDLOW, INITREDHIGH);
		Particles[i].g0 = Ranf(INITGREENLOW, INITGREENHIGH);
		Particles[i].b0 = Ranf(INITBLUELOW, INITBLUEHIGH);
		Particles[i].tr = Ranf(0.f, 1.f);
		Particles[i].tg = Ranf(0.f, 1.f);
		Particles[i].tb = Ranf(0.f, 1.f);
		Particles[i].t00 = Ranf(0.f, DURATION);
		Particles[i].t0 = Particles[i].t00;
		//Particles[i].t1 = Ranf(0.f, 1.f);
		Particles[i].t1 = Ranf(0.f, DURATION);
	}
