...
const char *WINDOWTITLE = { "Chain Demo -- Kevin Neiger" };
const char *GLUITITLE   = { "User Interface Window" };

#define ALLOW_PICKING

const float MASSMIN =  0.5f;
const float MASSMAX =  1.f;

const float KMIN =  20.f;
const float KMAX = 100.f;

const float CDMIN =  0.5f;
const float CDMAX =  3.00f;

const float GMIN = -20.f;
const float GMAX =  20.f;
...
const float LENGTH0 = 0.03f;

const float DTMIN = 0.001f;
const float DTMAX = 0.010f;
...
Animate( void )
{
	...
	if( Debug )
		fprintf(stderr, "Mass = %8.2f, Dt = %8.3f, Gravity = %8.2f, K = %8.2f, Cd = %8.2f\n", Mass, Dt, Gravity, K, Cd);
	...
	for( int node = 0; node < NUMNODES; node++ )
	{
		float aavg = ( Derivatives1[node].ay + Derivatives2[node].ay ) / 2.f;
		float vavg = ( Derivatives1[node].vy + Derivatives2[node].vy ) / 2.f;
		State[node].y  = State[node].y  + vavg * Dt;
		State[node].vy = State[node].vy + aavg * Dt;

		      aavg = ( Derivatives1[node].ax + Derivatives2[node].ax ) / 2.f;
		      vavg = ( Derivatives1[node].vx + Derivatives2[node].vx ) / 2.f;
...
Circle( float xc, float yc, float radius )
{
	glBegin( GL_TRIANGLE_FAN );
		glVertex2f( xc, yc );
		int numsegs = 15;
		float dang = 2.f*(float)M_PI/(float)numsegs;
		float ang = 0.;
		for( int i = 0; i <= numsegs; i++, ang += dang )
		{
			glVertex2f( xc + radius*cosf(ang), yc + radius*sinf(ang) );
		}
	glEnd( );
}
...
Display( void )
{
	...
	GLfloat scale2 = 1.f + Scale2;		// because glui translation starts at 0.
	if( scale2 < MINSCALE )
		scale2 = MINSCALE;
...
GetDerivs( struct state state[NUMNODES], struct derivatives derivs[NUMNODES] )
{
	for( int node = 0; node < NUMNODES; node++ )
	{
		...
		if( node == 0 )	 // node #0 is attached to node X0,Y0 by a spring
		{
			xm = X0 - state[node].x;
			ym = Y0 - state[node].y;
		}
		...
		{
			force = Cd * v;
			sumfx -= force * state[node].vx;	      // the damping
			sumfy -= force * state[node].vy;	      // the damping
		}
...
InitGlui( void )
{
	...
	slider = Glui->add_slider( false, GLUI_HSLIDER_FLOAT, &Cd );
    	slider->set_float_limits( (float)CDMIN, (float)CDMAX );
    	slider->set_w( 200 );
		//fprintf(stderr, "Damping: %8.3f, %8.3f   %8.3f\n", CDMIN, CDMAX, Cd);
		Glui->add_separator( );
...
MouseButton( int button, int state, int x, int y )
{
	...
	if( state == GLUT_DOWN )
	{
		...
		GLint xl = ( vx - v ) / 2;
		GLint yb = ( vy - v ) / 2;
		int xm = xl + (int)( (float)v * ( X0 - XL ) / ( XR - XL ) );
		int ym = yb + (int)( (float)v * ( Y0 - YB ) / ( YT - YB ) );
		ym = vy - ym;
		Picked = abs(x-xm) <= TOL  &&  abs(y-ym) <= TOL;
		if( Debug )
		{
			fprintf( stderr, "x  = %4d, y  = %4d\n", x, y );
			fprintf( stderr, "xm = %4d, ym = %4d\n", xm, ym );
...
MouseMotion( int x, int y )
{
	...
	if( ActiveButton & LEFT )
	{
		...
		if( Picked )
		{
			...
			GLint xl = ( vx - v ) / 2;
			GLint yb = ( vy - v ) / 2;
			X0 = XL + ( XR - XL ) * (float)( x - xl ) / (float)v;
			y = vy - y;
			Y0 = YB + ( YT - YB ) * (float)( y - yb ) / (float)v;
			if( Debug )
			{
				fprintf( stderr, "X0 = %8.3f, Y0 = %8.3f\n", X0, Y0 );
			}
		}
#endif
	}
...
Reset( void )
{
	...
	Y0 = 0.;
	Gravity = -9.8f;
	Dt = ( 3.f*DTMIN + DTMAX ) / 4.f;

	for( int node = 0; node < NUMNODES; node++ )
	{
		State[node].x = X0;
		State[node].vx = 0.;
		//State[node].y = Y0 - LENGTH0 * (float)(node+1);
		if( node == 0 )
